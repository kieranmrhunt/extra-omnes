...
function runTargetedChecks(variant, api) {
	...
	if (variant.label === "carafa-winter-1559") {
		...
		const nightA = api.initState("rebiba", "carafa-night-disparage", { headless: true });
		nightA.boost.medici = 2;
		const nightRisk = api.nightDisparageRisk1559("medici"), nightResultA = api.resolveNightDisparage1559("medici"), nightSnapshot = JSON.stringify(nightA);
		const nightB = api.initState("rebiba", "carafa-night-disparage", { headless: true });
		nightB.boost.medici = 2;
		const nightResultB = api.resolveNightDisparage1559("medici");
		assert(nightRisk > 0 && nightRisk < 1 && nightResultA.standingLoss > 0 && nightA.boost.medici < 2 && JSON.stringify(nightResultA) === JSON.stringify(nightResultB) && JSON.stringify(nightB) === nightSnapshot, "Carafa Winter: disparaging a rival is ineffective, invalid, or non-deterministic");
		...
		api.aiPolitics1559();
		const mediciAfterOneRealignment = state.boost.medici || 0;
		api.aiPolitics1559();
		assert(mediciAfterOneRealignment > mediciBefore && state.boost.medici === mediciAfterOneRealignment && state.flags.christmasRealignment, "Carafa Winter: late Medici pressure still compounds after every scrutiny");
		state.stage = 10;
		state.scrutinies.push({ number: 11, threshold: 30 });
		api.headlessAdvance1559();
		assert(state.stage === 11 && !state.over && state.ap === 3, "Carafa Winter: scrutiny XII still begins immediately after XI without a bargaining turn");
		const settlementRng = state.rng.state, settlement = api.settlementPlan1559();
		assert(settlement && settlement.id && settlement.readiness.need === api.threshold() && settlement.readiness.low <= settlement.readiness.high && state.rng.state === settlementRng, "Carafa Winter: final settlement forecast is missing, invalid, or consumes ballot randomness");
